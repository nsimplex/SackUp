...
BASE_SCRIPTS:=$(foreach f, configuration_schema.lua, src/$(f))
LIB_SCRIPTS:=$(foreach f, customizability.lua, lib/$(f))
CONFIGURATION_SCRIPTS:=rc.lua rc.defaults.lua
MISC_FILES:=AUTHORS.txt COPYING.txt $(ICON) $(ICON_ATLAS)

FILES:=$(CORE_FILES) $(BASE_SCRIPTS) $(LIB_SCRIPTS) $(CONFIGURATION_SCRIPTS) $(MISC_FILES)
